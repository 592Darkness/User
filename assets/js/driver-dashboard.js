...
function loadAvailableRides() {
    ...
    fetchWithLoader('/api/driver-available-rides.php', 'GET')
        .then(response => {
            if (response.success) {
                ...
                if (rides.length > 0) {
                    ...
                    rides.forEach(ride => {
                        ...
                        rideElement.innerHTML = `
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h4 class="text-lg font-medium text-white">${ride.distance.toFixed(1)} km Trip</h4>
                                    <p class="text-gray-400">${ride.pickup} → ${ride.dropoff}</p>
                                </div>
                                <div class="bg-primary-500/20 text-primary-400 px-3 py-1 rounded-full text-sm font-medium">
    ...
    fetchWithLoader(`/api/driver-earnings.php?period=${period}`, 'GET')
        .then(response => {
            if (response.success) {
                ...
                updateEarningsBreakdown(response.data.breakdown);
